...
def create_center_lines(path, cnn_predictions = True, patient_type = 'controls', compressed_sensing_data = False):
...
inf_results_dir = '/usr/bmicnas02/data-biwi-01/jeremy_students/lschlyter/CNN-segmentation/logdir/inference_results'
models = ['231115-1731_da_0.25_nchan4_r1_loss_dice_e125_bs8_lr0.001__tr_size_40_finetune',
...
for model in models:
    ...
    for patient_type in class_labels:
        ...
        all_seg_paths = [os.path.join(model_path, f) for f in os.listdir(model_path)]
        ...
        if not os.listdir(os.path.join('/',*model_path.split('/')[:-1])).__contains__('SubjectCenterlines'+suffix):
            create_center_lines(model_path, patient_type= patient_type,cnn_predictions= True, compressed_sensing_data= compressed_sensing_data)
        """
        ...
        if not os.path.exists(filepath_output):
            masked_sliced_data = prepare_and_write_masked_data_sliced_bern(model_path=model_path, filepath_output=filepath_output, patient_type=patient_type, suffix= suffix)
