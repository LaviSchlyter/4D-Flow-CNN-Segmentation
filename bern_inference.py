...
def save_prediction(prediction, prediction_sized, output_path):
    print('========================== Saving prediction to: {} ========================== '.format(output_path))
    # Save prediction
...
def run_inference(training_output_path, inference_input_path, label_path, inference_output_path, use_final_output_dir, final_model_output_file, patient_id):
    ...
    # ======================
    print('==============================================')
    print('Running inference with the following settings:')
    
    print('========================== Experiment name: {} ========================== '.format(exp_config.experiment_name))
    print('========================== Prediction segmentation for image: {} ========================== '.format(inference_input_path))

    # ======================
    # load best model
    # ======================
    print('========================== Looking for saved segmentation model... ========================== ')
    ...
    if os.path.exists(best_model_path):
        print('========================== Found best model from: {} ========================== '.format(best_model_path))
        # Load model
    ...
    # load data
    # ======================
    print('========================== Loading data from: {} ========================== '.format(inference_input_path))
    # Load data
    image = h5py.File(inference_input_path,'r')["data"]
    print('Shape of image before processing: {}'.format(image.shape))
    # Preprocess data
    ...
    if os.path.exists(label_path):
        ...
        print('Shape of label before processing: {}'.format(label.shape))
        if exp_config.slices == 'all':
        ...
        print('Shape of label after processing: {}'.format(label.shape))
        dice, mean_dice, mean_fg_dice = compute_dice(logits, label)
        print('Dice score: {}'.format(dice))
        print('Mean dice score: {}'.format(mean_dice))
        print('Mean foreground dice score: {}'.format(mean_fg_dice))
    ...
    print('Shape of prediction_sized after cropping or padding: {}'.format(prediction_sized.shape))

    # Permute back to [x, y, z, t, C]
    prediction = prediction.permute(1,2,0,3)

    print('Shape of prediction after permuting: {}'.format(prediction.shape))

    # ======================
    # save prediction
    # ======================
    if use_final_output_dir:
        print('========================== Saving prediction to final folder: {} ========================== '.format(final_model_output_file))
        
        save_prediction(prediction, prediction_sized, final_model_output_file)
    else:
        print('========================== Saving prediction to inference folder: {} ========================== '.format(inference_output_path))
